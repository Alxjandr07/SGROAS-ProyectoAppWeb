...
La lista de checksums SHA-256 está en `MANIFEST.csv` (272 archivos). Verificación:

```sh
certutil -hashfile <archivo> SHA256            # Windows
cd dataset && sha256sum -c MANIFEST.sha256     # Linux/macOS - rutas relativas a dataset/ (verificado 272 OK)
