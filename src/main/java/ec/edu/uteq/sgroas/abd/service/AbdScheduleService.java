...
@Transactional
public class AbdScheduleService {
    ...
    public AbdDtos.ScheduleResponse create(AbdDtos.ScheduleRequest request) {
        ...
        AbdDriver conductor = conductorAbdRepository.findById(request.idConductor())
                .orElseThrow(() -> new IllegalArgumentException("Driver no encontrado: " + request.idConductor()));
        if (!"Activo".equalsIgnoreCase(unidad.getEstado())) {
            throw new IllegalStateException("La unidad " + unidad.getPlaca() + " no esta activa");
        }
        Schedule programacion = Schedule.builder()
                .fecha(request.fecha())
